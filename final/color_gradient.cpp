...
public:
    explicit LGradientShader(
            const GPoint p0, const GPoint p1, const GColor* colors, const int count, tilemode mode)
...
std::unique_ptr<GShader> GCreateLinearGradient(
        GPoint p0, GPoint p1, const GColor colors[], int count, GShader::TileMode mode) {
    auto shader = new LGradientShader(p0, p1, colors, count, tilemodes[static_cast<int>(mode)]);
