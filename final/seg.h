...
void printSeg(const Seg &seg) {
    printf("Seg{%f, %f, %f, %f, %d} ((%f, %f) -> (%f, %f))\n",
           seg.y1, seg.y2, seg.curX, seg.k, seg.winding, seg.curX, seg.y1, gx2(seg), seg.y2);
}
...
Seg mkSegf(float x1, float y1, float x2, float y2) {
    if (y1 > y2) {
        auto r = mkSegf(x2, y2, x1, y1);
        r.winding = -1;
        return r;
    }
    return {y1, y2, x1, (x2 - x1) / (y2 - y1), 1};
}
