...
public:
    ...
    std::vector<std::tuple<float, int>> rayCast_(int y, std::vector<Seg>& segmented_segs) {
        std::vector<std::tuple<float, int>> xs = {};
        xs.reserve(2);
        int i = 0;
        int j = 0;
        ...
        while (it != segmented_segs.end()) {
            auto s = (*it);
            if (isIntersecting(y, s)) {
                xs.push_back(std::make_tuple(getIntersection(y, s), s.winding));
            }
            ++it;
        }
        return xs;
    ...
    void blt(const GPaint& paint, int y, int startX, int endX) {
        if (startX >= fDevice.width()) return;
        endX = std::min(endX, fDevice.width() - 1);
        ...
        if (paint.getShader() != nullptr) {
            ...
            GPixel* storage = static_cast<GPixel *> (calloc(cnt, sizeof(GPixel*)));
            shader->setContext(matrices.top());
            shader->shadeRow(rowL, y, cnt, storage);

            GPixel* nstorage = static_cast<GPixel *> (calloc(cnt, sizeof(GPixel*)));
            if (filter != nullptr) {
    ...
    void drawPath(const GPath& path, const GPaint& paint) override {
        ...
        v = edger.next(pts);
        while(v != GPath::kDone){
            pts[0] = matrices.top().mapPt(pts[0]);
            pts[1] = matrices.top().mapPt(pts[1]);
            pts[2] = matrices.top().mapPt(pts[2]);
            pts[3] = matrices.top().mapPt(pts[3]);
//            matrices.top().mapPoints(pts, 4);

            if (v == GPath::Verb::kCubic) {
                auto tups = mkCubic(pts[0], pts[1], pts[2], pts[3]);
                for (auto tup : tups) {
                    if (std::isfinite(tup.k)) {
                        edges.push_back(tup);
                    }
                }
            }  else if (v == GPath::Verb::kQuad) {
                auto md1 = 0.3333333 * pts[0] + 0.6666666 * pts[1];
                auto md2 = 0.3333333 * pts[2] + 0.6666666 * pts[1];
                auto tups = mkCubic(pts[0], md1, md2, pts[2]);
                for (auto tup : tups) {
                    if (std::isfinite(tup.k)) {
                        edges.push_back(tup);
                    }
                ...
                Seg tup = mkSegf(pts[0].x(), pts[0].y(), pts[1].x(), pts[1].y());
                if (std::isfinite(tup.k)) {
                    edges.push_back(tup);
                }
            }
        ...
        for (int y = 0; y < fDevice.height(); y ++) {
            ...
            std::sort(rc.begin(), rc .end(),
                      [](std::tuple<float, int> const& a, std::tuple<float, int> const& b) -> bool {
                          return std::get<0>(a) < std::get<0>(b);
                      }
            );

            int winding = std::get<1>(rc[0]);
            int i = 0;
            int curx = urnd(std::get<0>(rc[0]));
            for (std::tuple<float, int> e : rc) {
                if (i == 0) {
                    i ++;
                    continue;
                }
                int nx = urnd(std::get<0>(e));
                int nw = std::get<1>(e);
...
//                blt(paint, y,curx, nx);
    ...
    std::unique_ptr<GShader> final_createRadialGradient(GPoint center, float radius,
        ...
        auto shader = new RadialGradientShader(center,radius, colors, count);
        return std::unique_ptr<GShader>(shader);
    }

    std::unique_ptr<GShader> final_createTriangleGradient(const GPoint pts[3],
                                                          const GColor colors[3]) {
        auto shader = new TriangleGradientShader(pts, colors);
        return std::unique_ptr<GShader>(shader);
    }

    void final_addStrokedLine(GPath* path, GPoint p0, GPoint p1,
                              float width, bool roundCap) {
        addStrokedLine(path, p0, p1, width, roundCap);
    }
...
//    printAllSegs(c2);
    ...
    printAllSegs(mkCubic({0, 0}, {100, 100}, {200, 100}, {300, 0}));
    printf("==== TESTENDD ====\n");
}
