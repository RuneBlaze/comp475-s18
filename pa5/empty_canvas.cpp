...
class PointTup {
public:
    explicit PointTup(GPoint lhs, GPoint rhs) {
        if (rhs.y() > lhs.y()) {
            this->top = rhs;
            this->bottom = lhs;
            this->winding = 1;
    ...
    bool cmp(const PointTup& rhs) {
        // TODO: fix this to the correct version
        if (this->top.y() < rhs.top.y()) return true;
        return false;
    }
...
//            }
    ...
    void drawPath(const GPath& path, const GPaint& paint) override {
        ...
        while(edger.next(pts) != GPath::kDone) {
            pts[0] = matrices.top().mapPt(pts[0]);
            pts[1] = matrices.top().mapPt(pts[1]);
            auto tup = mkSegf(pts[0].x(), pts[0].y(), pts[1].x(), pts[1].y());
