...
//                auto sd = paint.getShader();
//                auto cnt = r.right() - r.left();
//                GPixel storage[cnt];
//                sd->shadeRow(r.left(),y, cnt, storage);
//                blendRow(fn, r.left(), y, storage, cnt);
//            } else {
...
//        }
    ...
    void drawConvexPolygon(const GPoint pts_[], int count, const GPaint& paint) override  {
        if (count <= 1) return;
        GPoint pts[count];
        ctms.top().mapPoints(pts, pts_, count);
...
//            }
    ...
    void concat(const GMatrix& matrix) override {
        ctms.top().preConcat(matrix);
    }

private:
    const GBitmap fDevice;
    std::stack<GMatrix> ctms;
};
